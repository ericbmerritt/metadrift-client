...
parseCreate =
  ...
  (T.pack <$>
   strOption
     (long "username" <> short 'u' <> metavar "STRING" <>
      help "The identifying name for this user")) <*>
  (T.pack <$>
   strOption
     (long "preferred-name" <> short 'p' <> metavar "STRING" <>
      help "The prefered name for use in 'normal name' situations")) <*>
  (T.pack <$>
   strOption
     (long "email" <> short 'e' <> metavar "EMAIL" <>
      help "The email address of the user")) <*>
  many
    (T.pack <$>
     strOption
       (long "team" <> short 't' <> metavar "STRING" <>
        help "A name of a team that the user is a part of"))
...
addMap =
  Map.fromList
    ...
    , ( "teams"
      , \val obj ->
          let newVal = val : Lens.getL Service.User._teams obj
  ...
  case Map.lookup action actionMap of
    Just aMap ->
      Support.setField aMap user fieldName value >>= \case
        Just newUser ->
          Service.patchUser config user newUser >>= Support.printBody
...
doCommand config (Get gid) = Service.getUser config gid >>= Support.printBody
...
main cfg args = do
  ...
  where
    opts =
      info
        (helper <*> parseCommand)
        (fullDesc <> progDesc "Manage a user in the system" <>
