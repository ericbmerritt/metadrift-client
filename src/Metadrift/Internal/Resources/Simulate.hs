...
commandParser = Command <$> option auto
                              (long "percentile"
                               <> short 'p'
                               <> metavar "PERCENTILE"
                        ...
                        <*> many
                              (strOption
                                 (long "tag"
                                  ...
                                  <> metavar "TAG"
                                  <> help
                                       "the tag to identify cards to simulate for"))
                        <*> many
                              (strOption
                                 (long "workflow"
                                  <> short 'w'
  ...
  Service.simulate config
    Service.Simulate.T
      ...
      , Service.Simulate.workflows = listToMaybe $ map
                                                     (Service.Card.stringToWorkflow . T.pack)
                                                     workflow
      } >>= Support.printBody

main :: Service.Config -> [T.Text] -> IO ExitCode
main cfg args = do
  cmd <- Utils.handleParseResult "metad simulate"
  ...
  where
    opts = info (helper <*> commandParser)
             (fullDesc
              <> progDesc "Run a simulation for the specified card group"
