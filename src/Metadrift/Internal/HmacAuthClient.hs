-- ----------------------------------------------------------------------- |
-- Module : Network.Wai.Middleware.HmacAuth.Client Description : Wai HMAC Auth
-- Middleware Client Copyright : (c) 2015 Christopher Reichert License : BSD3
-- Maintainer : Christopher Reichert <creichert07@gmail.com> Stability :
-- experimental Portability : POSIX Compatible with HTTP Client
...
-- | Add an Hmac auth header, signed with the specified secret, to the given
-- Request. Ignore error handling:
--
...
--
-- http-request uri from the protocol name up to the query string. TODO add the
-- query string to the canonicalized resource?
